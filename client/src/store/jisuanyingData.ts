...
export const JISUANYING_CROSS_BRAND_DATA = {
  ...
  brandSummaries: {
    计算营: '家长最看重**即时批改、深度诊断和专项练习**；当前较明确的挑战是效果感知和孩子坚持，部分开放反馈也提示老师服务体验仍有改进空间。',
    '教辅/题卡': '是使用率最高、价格锚点较低的方案；主要负担是**家长仍需监督和批改**，训练效果也不易判断。',
    '计算App': '能够提供数字化练习和即时反馈；用户的不满主要集中在**孩子难坚持、效果不明显和仍需家长介入**。',
    综合数学班: '用户已经建立教育付费习惯；但计算专项训练和个性化错因诊断**可能不够集中**。',
    '1v1私教': '可以提供高密度的个性化诊断与反馈；问卷中使用率较低，案头案例显示其**价格和人工交付成本较高**。',
  },
};
