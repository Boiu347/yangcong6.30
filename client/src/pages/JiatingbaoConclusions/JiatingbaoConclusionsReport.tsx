...
import {
  ...
  type SubSection,
  type VocClip,
} from './jiatingbaoConclusionsData';

const JTB_CONCLUSIONS_STORE_KEY = 'jiatingbao-conclusions';

const INTERVIEW_SOURCE_URL = 'https://guanghe.feishu.cn/wiki/U7MvwNaI9iLmABkJdiLclBOinGc';
const RESEARCH_SOURCE_URL = 'https://guanghe.feishu.cn/wiki/SW7FwtXejisq42kR241cdjuPnAd';
const INSIGHT_SOURCE_URL = 'https://guanghe.feishu.cn/wiki/V9utwegDYipqxIkslIfcTL0Inud';
...
function VocClipCard({ clip, color }: { clip: VocClip; color: string }) {
  ...
  return (
    <div data-evidence-card className="flex w-full flex-col rounded-[12px] border border-[#EADFD2] bg-white px-3.5 py-3">
...
function PointBlock({ point, color }: { point: Point; color: string }) {
  return (
    <div className="rounded-[12px] px-3.5 py-3.5" style={{ backgroundColor: '#FFF9F5' }}>
...
export default function JiatingbaoConclusionsReport() {
  const editor = useIsEditor();
  const { data: storedSections, saving, save } = useContentStore<DimensionSection[]>(
    JTB_CONCLUSIONS_STORE_KEY,
    jiatingbaoConclusionSections,
  );
  const sections = storedSections.length > 0 ? storedSections : jiatingbaoConclusionSections;

  const [selectedByDimension, setSelectedByDimension] = React.useState<Record<string, string>>(() =>
    jiatingbaoConclusionSections.reduce(
      (acc, section) => {
  ...
  const [activeSection, setActiveSection] = React.useState(jiatingbaoConclusionSections[0]?.id ?? '');

  const totalVoc = countJtbVocClips();
  const userCount = countJtbUsers();
  ...
  return (
    <main className="min-h-full bg-[#F8F6F1] text-[#292521]">
        ...
        <div className="mx-auto max-w-[1440px]">
          <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
            ...
            <div className="grid w-full max-w-[420px] grid-cols-2 gap-2 lg:w-[420px] lg:shrink-0">
              {[
                { label: '决策洞察', url: INSIGHT_SOURCE_URL },
                { label: '研究方案', url: RESEARCH_SOURCE_URL },
                { label: '访谈纪要', url: INTERVIEW_SOURCE_URL },
              ].map((link) => (
                <a
                  key={link.label}
                  href={link.url}
                  target="_blank"
                  rel="noreferrer"
                  className="inline-flex items-center justify-center gap-1.5 rounded-full border border-[#D8D0C6] bg-white px-3 py-2 text-[12px] font-black text-[#5F5851] shadow-sm transition hover:border-[#E8643C] hover:text-[#E8643C]"
                >
                  <FileText size={13} />
                  {link.label}
          ...
          <div className="mt-7 grid gap-4 md:grid-cols-3">
            {[
              { icon: Lightbulb, value: totalConclusions, label: '核心结论', desc: '成交机制到销售打法', color: '#E8643C', bg: '#FFF3EE' },
              { icon: Quote, value: totalVoc, label: '访谈原声', desc: '与结论强绑定的原话', color: '#2F9F8F', bg: '#EFFFFB' },
              { icon: Target, value: userCount, label: '涉及用户', desc: '覆盖已购/未购家庭', color: '#C58A3D', bg: '#FFF7E8' },
      ...
      <section className="px-5 pb-8 pt-6 md:px-8">
        <div className="mx-auto max-w-[1440px] space-y-6">
          {sections.map((section) => {
            const Icon = iconByKey[section.iconKey] ?? Lightbulb;
      ...
      <footer className="px-5 pb-8 text-center text-[12px] font-semibold text-[#A19990] md:px-8">
        内容对应《家庭包用户购买决策洞察》；访谈原声取自《【访谈纪要】家庭包用户调研》中的 P1-P10 家庭。
      </footer>
